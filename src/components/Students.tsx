...
import { Plus, Edit2, Trash2, Upload, Download, FileSpreadsheet, AlertCircle, Archive } from 'lucide-react';
...
export function Students() {
  ...
  const handleDeactivate = async (id: string, name: string) => {
    if (!confirm(`Deactivate "${name}"? Their attendance will be archived to Past Student Records. This cannot be undone from the Students page.`)) {
      return;
    }

    try {
      const response = await fetch(`${API_BASE_URL}/api/students/${id}/deactivate`, {
        method: 'POST',
      });
      const result = await response.json();

      if (response.ok && result.success) {
        alert('✅ Student deactivated and moved to Past Records!');
        await fetchStudents();
      } else {
      ...
      alert(`❌ Error: ${error instanceof Error ? error.message : 'Failed to deactivate student'}`);
    }
  };
                  ...
                  <td className="py-3 px-4">
                    <div className="flex gap-2">
                      ...
                      <button
                        ...
                        title="Deactivate & archive to Past Records"
                      >
