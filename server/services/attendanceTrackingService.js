...
class AttendanceTrackingService {
  ...
  // Mark attendance for a student - FIXED VERSION
  async markAttendance(studentId, studentName, studentEmail, date, status, className) {
    try {
      ...
      if (existingRecordsForDate.length > 0) {
        console.log(`🔄 Found ${existingRecordsForDate.length} existing record(s) - deleting...`);
        
        for (const record of existingRecordsForDate) {
          await mongoService.deleteAttendanceRecord(record.id);
        }
        console.log(`✅ Deleted ${existingRecordsForDate.length} old record(s)`);
      }

      // Create timestamp for the selected date at noon UTC
      ...
      console.log(`💾 Saving to database:`, {
        ...
        matches: new Date(record.timestamp).toISOString().split('T')[0] === date ? '✅' : '❌'
      });
