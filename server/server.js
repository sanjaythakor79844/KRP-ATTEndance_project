...
import pastRecordsService from './services/pastRecordsService.js';
...
// Deactivate student and archive for Past Student Records
app.post('/api/students/:id/deactivate', async (req, res) => {
  try {
    ...
    const result = await mongoService.deactivateStudent(req.params.id);
    if (!result.success) {
    ...
    await mongoService.addLog({
      ...
      details: `Student ${student.name} (${student.id}) deactivated and archived to Past Records`,
      category: 'past_records_audit',
      userId: req.headers['x-user-id'] || 'admin',
    });

    const updatedStudent = await mongoService.getStudentById(req.params.id);
    res.json({ success: true, data: updatedStudent, message: 'Student deactivated and archived successfully' });
  } catch (error) {
    res.status(500).json({ success: false, error: error.message });
  }
});
...
app.get('/api/records/students', async (req, res) => {
  try {
    ...
    await pastRecordsService.logRecordsAccess(
      'Past Records List Viewed',
      `Viewed past student list (page ${pagination.page}, search: "${search || 'none'}")`,
...
app.get('/api/records/students/:id/attendance', async (req, res) => {
  try {
    ...
    const records = await pastRecordsService.getStudentAttendanceRecords(req.params.id);

    await pastRecordsService.logRecordsAccess(
      'Past Student Attendance Viewed',
      `Viewed attendance records for ${student.name} (${student.id})`,
      req.headers['x-user-id'] || 'admin'
    );
...
app.get('/api/records/students/:id/attendance/summary', async (req, res) => {
  try {
    ...
    await pastRecordsService.logRecordsAccess(
      ...
      `Viewed attendance summary for ${student.name} (${student.id}) filter=${filterMode}`,
      req.headers['x-user-id'] || 'admin'
    );
